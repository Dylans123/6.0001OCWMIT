...
def get_word_score(word, n):
    ...
    for x in word:
        if x == "*":
            ...
            x = SCRABBLE_LETTER_VALUES[x]
            first.append(x)
    second = (7 * len(word) - 3 * (n - len(word)))
    if second <= 0:
        second = 1
    word_score = sum(first) * second
    return word_score
...
def calculate_handlen(hand):
    ...
    """
    length = 0
    hand_length = hand.values()
    for x in hand_length:
        length += x
    return length
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) >= 0:
        ...
        if word == "!!":
            ...
            if is_valid_word(word, hand, word_list) == True:
                ...
                print(word, "earned", get_word_score(word, n), "points. Total score: ", Total_score)
            else:
    ...
    print("Total score after this hand: ", Total_score)
...
def substitute_hand(hand, letter):
    ...
    letters_to_pull_from = []
    hand2 = hand.copy()
    hand3 = hand.keys()
    for x in alphabet:
        if x not in hand3:
            letters_to_pull_from.append(x)
    if letter in hand2:
        val = hand2.get(letter)
        del(hand2[letter])
        newchoice = random.choice(letters_to_pull_from)
        hand2[newchoice] = val
    return hand2
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    """
    n = HAND_SIZE
    Handnum = int(input("Enter a total number of hands: "))
    while Handnum > 0:
        Current_score = 0
        ...
        print("-------")
        replay = input("Would you like to replay that hand? (yes/no): ")
        if replay == "yes":
            Current_score = 0
            current_hand = hand
            Current_score = play_hand(hand, word_list)
        if replay == "no":
            pass
        Total_score_over_all_hands += Current_score
        Handnum -= 1
    print("Total score over all hands: ", Total_score_over_all_hands)
#
