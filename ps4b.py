...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for k in range(0, len(string.ascii_lowercase)):
            cd[string.ascii_lowercase[k]] = string.ascii_lowercase[(k + shift) % len(string.ascii_lowercase)]
        for k in range(0, len(string.ascii_uppercase)):
            cd[string.ascii_uppercase[k]] = string.ascii_uppercase[(k + shift) % len(string.ascii_uppercase)]
        return cd
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        big_test = []
        for x in range(26):
            adjusted_text = self.apply_shift(x)
            ...
            for word in adjusted_words:
                if is_word(word_list, word):
                    ...
                    test.append(0)
            big_test.append((sum(test), x, adjusted_text))
            del(test[0:len(test)])
